...
import { Phone, Fax, Mail, Menu, X, ChevronDown, Facebook, Youtube, Linkedin } from "lucide-react";
import { navigation, contactInfo, socialLinks } from "@/content/navigation";
import type { NavItem } from "@/content/navigation";
import logoImg from "@/assets/logo-cardio.png";
...
const DropdownMenu = ({ item, isActive }: { item: NavItem; isActive: boolean }) => {
  ...
  return (
    <div className="relative" onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)}>
      <button
        className={`flex items-center gap-1 px-1 py-2 text-sm font-medium transition-colors hover:text-primary ${isActive ? "text-primary" : "text-foreground"}`}
      >
        {item.label}
        ...
        <div className="absolute top-full left-0 mt-0 w-64 rounded-lg border bg-card shadow-lg py-2 z-50">
          {item.children?.map((child) => (
            <Link
              key={child.href}
              to={child.href}
              className="block px-4 py-2.5 text-sm text-body hover:bg-section hover:text-primary transition-colors"
    ...
    <header className="sticky top-0 z-50 border-b bg-background/80 backdrop-blur-md">
      ...
      <nav className="container-main flex items-center justify-between h-16 lg:h-20">
        ...
        <div className="hidden lg:flex items-center gap-6">
          ...
          <Link
            to="/contact"
            className="ml-2 inline-flex items-center gap-2 bg-primary text-primary-foreground px-6 py-2.5 rounded-lg text-sm font-medium hover:opacity-90 transition-opacity"
          >
            Contactez Nous
          </Link>
        </div>
        ...
        <div className="lg:hidden border-t bg-background">
          <div className="container-main py-4 space-y-1">
            {navigation.map((item) =>
              item.children ? (
                    ...
                    <div className="pl-4 space-y-1">
                      {item.children.map((child) => (
                        <Link
                          key={child.href}
                          to={child.href}
                          className="block px-3 py-2 text-sm text-body hover:text-primary"
                ...
                <Link
                  key={item.href}
                  to={item.href}
                  className={`block px-3 py-2.5 text-sm font-medium ${isActive(item) ? "text-primary" : "text-foreground"}`}
                  onClick={() => setMobileMenuOpen(false)}
                >
                  {item.label}
                </Link>
            ...
            <div className="pt-3">
              <Link
                to="/contact"
                className="block text-center bg-primary text-primary-foreground px-6 py-2.5 rounded-lg text-sm font-medium"
                onClick={() => setMobileMenuOpen(false)}
              >
                Contactez Nous
              </Link>
