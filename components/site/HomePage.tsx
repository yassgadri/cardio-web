      ...
      <section className="relative min-h-[80vh] flex items-center overflow-hidden">
        ...
        <div className="container-main relative z-10 py-20">
          <motion.div className="max-w-2xl" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-semibold text-primary-foreground leading-tight">Service de Cardiologie</h1>
            <p className="text-xl md:text-2xl text-primary-foreground/80 mt-4 font-medium">Clinique Sainte Clotilde</p>
            <div className="mt-8 flex flex-wrap gap-4">
              <Link href="/qui-sommes-nous" className="inline-flex items-center gap-2 bg-background text-primary px-8 py-3 rounded-lg text-sm font-semibold hover:bg-background/90 transition-colors">
                Nos spécialités <ArrowRight className="h-4 w-4" />
              </Link>
              <a href={`tel:${contactInfo.phone.replace(/\s/g, '')}`} className="inline-flex items-center gap-2 border border-primary-foreground/30 text-primary-foreground px-8 py-3 rounded-lg text-sm font-medium hover:bg-primary-foreground/10 transition-colors">
            ...
            {serviceCategories.map((cat, i) => {
              ...
              return (
                <motion.div key={cat.slug} className="medical-card group" {...fadeUp} transition={{ ...fadeUp.transition, delay: i * 0.1 }}>
                  ...
                  <Link href={`/${cat.slug}`} className="inline-flex items-center gap-2 text-primary text-sm font-medium group-hover:gap-3 transition-all">
        ...
        <div className="container-main">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div {...fadeUp}>
              ...
              <h2 className="text-2xl md:text-3xl lg:text-4xl mb-6">Service de cardiologie de la Clinique Sainte Clotilde</h2>
              ...
              <div className="flex flex-wrap gap-4">
                <Link href="/qui-sommes-nous" className="inline-flex items-center gap-2 bg-primary text-primary-foreground px-6 py-2.5 rounded-lg text-sm font-medium hover:opacity-90 transition-opacity">
                  Cliquez Ici
                </Link>
                <a href={`tel:${contactInfo.phone.replace(/\s/g, '')}`} className="inline-flex items-center gap-2 border border-border text-foreground px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-section transition-colors">
                  <Phone className="h-4 w-4" /> {contactInfo.phone}
                </a>
              </div>
            </motion.div>
        ...
        <div className="container-main">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-3xl mx-auto">
            <motion.div className="medical-card" {...fadeUp}>
              <Link href="/consultation" className="text-primary font-semibold text-lg mb-2 block hover:underline">Pour une prise de rendez-vous</Link>
              <p className="text-body text-sm mb-4">veuillez nous contacter</p>
              <Link href="/consultation" className="inline-flex items-center gap-2 text-primary text-sm font-medium">Lire la suite <ArrowRight className="h-4 w-4" /></Link>
            </motion.div>
            <motion.div className="medical-card" {...fadeUp} transition={{ ...fadeUp.transition, delay: 0.1 }}>
              <Link href="/hospitalisation" className="text-primary font-semibold text-lg mb-2 block hover:underline">Le secrétariat du service hospitalisation</Link>
              <p className="text-body text-sm mb-4">Pour toute information</p>
              <Link href="/hospitalisation" className="inline-flex items-center gap-2 text-primary text-sm font-medium">Lire la suite <ArrowRight className="h-4 w-4" /></Link>
            </motion.div>
          </div>
        </div>
      </section>
      ...
      <section className="section-padding">
        <div className="container-main text-center">
          <motion.p className="text-body text-lg" {...fadeUp}>
            ...
            <Link href="/contact" className="text-primary font-semibold hover:underline">nous contacter</Link>.
          </motion.p>
        </div>
      </section>
    </Layout>
