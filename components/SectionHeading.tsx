...
const SectionHeading = ({ overline, title, centered = true }: SectionHeadingProps) => (
  ...
  >
    {overline && <p className="text-primary text-sm font-semibold uppercase tracking-wider mb-2">{overline}</p>}
    <h2 className="text-2xl md:text-3xl lg:text-4xl">{title}</h2>
    {!centered && <div className="mt-4 h-1 w-16 rounded-full bg-primary" />}
  </motion.div>
);
